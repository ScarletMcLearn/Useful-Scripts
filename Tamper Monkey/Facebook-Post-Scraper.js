// ==UserScript==
// @name         Post Extractor Widget (FB: Date Range + CSV) - Robust + Debug (Collapsible)
// @namespace    https://example.com/
// @version      0.6.2
// @description  Extract FB post permalink/text/image/date while scrolling; robust selectors; stop logic for non-chronological feeds; debug logs; download CSV. Collapsible launcher.
// @match        *://facebook.com/*
// @match        *://*.facebook.com/*
...
(() => {
  ...
  try {
     ...
     * CONFIG
     ********************/
    const URL_REGEX = /\/(search\/|groups\/)/i;
    ...
    const SCROLL_POLL_MAX = 10;
    const SCROLL_RETRY_MAX = 3;
    ...
    const UI_BOTTOM_OFFSET_PX = 96; // avoid FB chat bubble overlay (bottom-right)

    /********************
     * GUARDS
     ********************/
    // FB is SPA; sometimes location changes after load — still inject only on groups/search.
    if (!URL_REGEX.test(location.pathname + location.search)) {
      // Still allow groups/search deep links that land after SPA routing
      // We'll retry a little later once.
      setTimeout(() => {
        if (document.getElementById("tm-extract-widget") || document.getElementById("tm-extract-launcher")) return;
        if (!URL_REGEX.test(location.pathname + location.search)) return;
        location.reload(); // simplest recovery if SPA navigated after our idle hook
      }, 1200);
      return;
    }
    if (document.getElementById("tm-extract-widget") || document.getElementById("tm-extract-launcher")) return;
    ...
    addStyle(`
      #tm-extract-widget, #tm-extract-widget * ,
      #tm-extract-launcher, #tm-extract-launcher * {
      ...
      #tm-extract-launcher{
        ...
        bottom: ${UI_BOTTOM_OFFSET_PX}px !important;
      ...
      #tm-extract-launcher:hover{ background: rgba(35,35,35,0.95); }
      #tm-launcher-title{
        font-size: 12px;
        font-weight: 700;
      ...
      #tm-extract-widget .tm-head .tm-title{
        font-size: 14px;
        font-weight: 800;
        letter-spacing: 0.2px;
        margin: 0;
      ...
      #tm-extract-widget .tm-head button{
        flex: unset;
        padding: 6px 10px;
        font-size: 12px;
        border-radius: 8px;
      ...
      #tm-extract-widget .tm-head button:hover{ background: rgba(255,255,255,0.16); }
     ...
     ********************/
    (async () => {
      const ok = await ensureBody();
      if (!ok) {
        console.error("[PostExtractor] No document.body after waiting.");
        return;
      }

      const launcher = document.createElement("div");
      launcher.id = "tm-extract-launcher";
      launcher.innerHTML = `
        <div id="tm-launcher-title">Post Extractor</div>
        <div id="tm-launcher-badge">0</div>
      `;
      ...
      widget.innerHTML = `
        <div class="tm-head">
          <div class="tm-title">Post Extractor</div>
          <button id="tmCollapseBtn" type="button" title="Collapse">—</button>
        </div>
       ...
       * WIRED ELEMENTS
       ********************/
      const $launcherBadge = launcher.querySelector("#tm-launcher-badge");
      const $collapseBtn = widget.querySelector("#tmCollapseBtn");
      ...
      function setCollapsed(isCollapsed) {
        widget.style.display = isCollapsed ? "none" : "block";
        launcher.style.display = isCollapsed ? "flex" : "none";
      }

      setCollapsed(START_COLLAPSED);
      ...
      function refreshBadge() { $launcherBadge.textContent = String(state.results.length); }
      ...
      async function scrollAndWaitForGrowth(beforeCount) {
        ...
        if (!grew) {
          ...
          grew = findCandidatePostAnchors().length > beforeCount;
        }
      ...
      function shouldStop() {
        if (state.stagnation >= STAGNATION_LIMIT) return true;
      ...
      async function runExtraction() {
        ...
        dbg("run:done", { results: state.results.length });
        setStatus(
          `Done.\n` +
          `Results: ${state.results.length}\n` +
          `Processed unique posts: ${state.seenKeys.size}\n` +
      ...
      console.log("[PostExtractor] UI injected OK:", location.href);
      dbg("init:ready", { href: location.href });
    })();

  } catch (e) {
    console.error("[PostExtractor] Fatal error:", e);
  }
})();
